...
async function build () {
  ...
  outputPath = path.join(paths.dist, 'ree-validate.minimal.js');
  fs.writeFileSync(outputPath, code);
  stats = utils.stats({ code, path: outputPath });
  console.log(`${chalk.green('Output File:')} ree-validate.minimal.js ${stats}`);
  ...
  outputPath = path.join(paths.dist, 'ree-validate.minimal.min.js');
  fs.writeFileSync(outputPath, uglify.minify(code, uglifyOptions).code);
  stats = utils.stats({ code, path: outputPath });
  console.log(`${chalk.green('Output File:')} ree-validate.minimal.js ${stats}`);
}
