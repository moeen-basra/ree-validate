...
async function build () {
  ...
  let outputPath = path.join(paths.dist, 'ree-validate.esm.js');
  fs.writeFileSync(outputPath, code);
  let stats = utils.stats({ code, path: outputPath });
  console.log(`${chalk.green('Output File:')} ree-validate.esm.js ${stats}`);

  bundle = await rollup(configs.esmMinimal.input);
  code = (await bundle.generate(configs.esmMinimal.output)).code;

  outputPath = path.join(paths.dist, 'ree-validate.minimal.esm.js');
  fs.writeFileSync(outputPath, code);
  stats = utils.stats({ code, path: outputPath });
  console.log(`${chalk.green('Output File:')} ree-validate.minimal.esm.js ${stats}`);
