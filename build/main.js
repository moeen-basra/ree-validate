...
async function build() {
  ...
  fs.writeFileSync(path.join(config.outputFolder, 'ree-validate.js'), code)
  console.log(chalk.green('Output File:') + ' ree-validate.js')

  fs.writeFileSync(path.join(config.outputFolder, 'ree-validate.min.js'), uglify.minify(code, config.uglifyOptions).code)
