...
async function build () {
  ...
  fs.writeFileSync(path.join(config.outputFolder, 'ree-validate.minimal.js'), code)
  console.log(chalk.green('Output File:') + ' ree-validate.minimal.js')

  fs.writeFileSync(path.join(config.outputFolder, 'ree-validate.minimal.min.js'), uglify.minify(code, config.uglifyOptions).code)
