...
test('adds field items to the collection', () => {
  ...
  expect(() => {
    bag.push(field);
  }).toThrowError(`[ree-validate] Field with id ${field.id} is already added.`);

  expect(() => {
    field.id = null;
    bag.push(field);
  }).toThrowError('[ree-validate] Field id must be defined.');

  // test type check.
  expect(() => {
    bag.push('generic');
  }).toThrowError('[ree-validate] FieldBag only accepts instances of Field');
});
